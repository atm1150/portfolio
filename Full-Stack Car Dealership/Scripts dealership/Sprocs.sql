...
AS
BEGIN
	SELECT PurchaseID, PurchaseType, PurchasePrice, PurchaseDate, CustomerID, UserID, VehicleID
	FROM Purchase
END
...
AS
BEGIN
	SELECT TransmissionID, TransStyle
	FROM Transmission
	ORDER BY TransStyle
END
...
BEGIN
	...
	VehicleType, ImageFileName, BodyStyleID, TransmissionID, BodyColorID, InteriorColorID, DateAdded, IsAvailable, IsFeatured
	FROM Vehicle
END
...
AS
BEGIN
	INSERT INTO Vehicle(VehicleYear, MakeID, ModelID, Price, Mileage, Vin, MSRP, VehicleDescription, VehicleType, ImageFileName, BodyStyleID, TransmissionID, BodyColorID, InteriorColorID, IsAvailable, IsFeatured)
	VALUES (@VehicleYear, @MakeID, @ModelID, @Price, @Mileage, @Vin, @MSRP, @VehicleDescription, @VehicleType, @ImageFileName, @BodyStyleID, @TransmissionID, @BodyColorID, @InteriorColorID, @IsAvailable, @IsFeatured);
...
AS
BEGIN
	INSERT INTO Purchase(PurchaseType, PurchasePrice, CustomerID, UserID, VehicleID)
	VALUES (@PurchaseType, @PurchasePrice, @CustomerID, @UserID, @VehicleID);
...
BEGIN
	...
	WHERE VehicleType = @VehicleType AND IsAvailable = 1
...
AS
BEGIN
	SELECT VehicleID, VehicleYear, MakeID, ModelID, Price, Mileage, Vin, MSRP, VehicleDescription, VehicleType, ImageFileName, BodyStyleID, TransmissionID, BodyColorID, InteriorColorID, DateAdded, IsAvailable, IsFeatured
	FROM Vehicle
	WHERE VehicleID = @VehicleID
END
...
AS
BEGIN
	SELECT VehicleID, VehicleYear, mk.MakeName, md.ModelName, Price, Mileage, Vin, MSRP, VehicleDescription, VehicleType, ImageFileName, bs.BodyDescription, t.TransStyle, BodyColorName, InteriorColorName, IsAvailable
	FROM Vehicle v
